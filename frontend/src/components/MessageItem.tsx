...
// FIX: Removed faIR locale to default to English
// import { faIR } from 'date-fns/locale'

const formatTimestamp = (ts: number) => {
  try {
    // FIX: Removed { locale: faIR }
...
export const MessageItem: React.FC<{ message: ChatMessage }> = ({ message }) => {
  ...
  // Text direction detection is still useful
  ...
  return (
    <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
      <div
        dir={textDir}
        className={`flex flex-col p-3 rounded-2xl max-w-[90%] md:max-w-[75%] ${isUser ? 'bg-primary text-primary-foreground rounded-br-lg' : 'bg-surface text-surface-foreground rounded-bl-lg'
      ...
      >
        <div className="flex items-center gap-2 mb-2 text-xs font-semibold">
          ...
          <span className={isUser ? 'text-primary-foreground/80' : (error ? 'text-destructive' : 'text-surface-foreground/80')}>
            {/* Translated roles */}
            {isUser ? 'You' : 'Cando Assistant'}
          </span>
        </div>

        {/* Added prose-sm for better default text styling */}
        <div className={`prose prose-sm dark:prose-invert prose-p:my-0 prose-pre:my-2 whitespace-pre-wrap break-words ${isUser ? 'text-primary-foreground' : (error ? 'text-destructive' : 'text-surface-foreground')
          }`}>
          {text || (error ? '' : '...')}
        </div>
        ...
        <div className="flex items-center gap-3 mt-2 self-end">
          ...
          {!error && text && (
            <button
              ...
              className={`p-1 rounded-full ${isUser
          ...
          {error && !isUser && (
            // Translated label
            <button onClick={handleRetry} aria-label="Retry" className="p-1 rounded-full text-destructive hover:bg-destructive/20 transition-colors">
              <RotateCw size={14} />
            </button>
          )}
        </div>
        ...
        {error && (
          // Set direction to LTR for English error messages
          <div dir="ltr" className="flex items-center gap-2 mt-2 text-xs text-destructive border-t border-destructive/30 pt-2">
...
// ... (TypingIndicator and MessageSkeleton remain the same)
